...
const situationByCategory: Record<EnglishExpression["category"], string[]> = {
  ...
  Business: [
    "회의나 메신저에서 짧게 답할 때",
    "상대에게 일정/업무 상황을 자연스럽게 전달할 때",
  ],
  Travel: [
    "호텔, 카페, 공항처럼 바로 말해야 하는 상황",
    "정중하지만 너무 딱딱하지 않게 요청할 때",
  ],
  Developer: [
    "버그 리포트, 코드 리뷰, 제품 논의 중",
    "문제를 설명하거나 해결 방향을 물어볼 때",
  ],
  OPIc: [
    "경험을 말하면서 자연스럽게 이유를 붙일 때",
    "답변을 길게 이어가기 전에 핵심 문장을 던질 때",
  ],
};
...
export default function ExpressionDetailScreen() {
  ...
  const isFavorite = useAppStore((state) =>
    selectIsExpressionSaved(state, expression.id),
  );
  ...
  const extraExamples = useMemo(
    () => getExtraExamples(expression),
    [expression],
  );
        ...
        <View style={styles.detailCard}>
          ...
          <View style={styles.metaPill}>
            <Text style={styles.metaText}>
              {expression.category} · {expression.level}
            </Text>
          </View>
        </View>
